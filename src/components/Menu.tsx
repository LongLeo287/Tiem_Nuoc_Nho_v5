...
import { Plus, Minus, X, Check, Search, Heart, AlertCircle, RefreshCw, ChevronRight, ShoppingBag, Settings as SettingsIcon, Package, ArrowUp, LayoutGrid, List, Coffee } from 'lucide-react';
...
export function Menu({ appsScriptUrl, onNavigateSettings }: MenuProps) {
  ...
  const [quantityItem, setQuantityItem] = useState<GroupedMenuItem | null>(null);
  ...
  useEffect(() => {
    setIsFabHidden(!!selectedItem || !!outOfStockItem || !!quantityItem);
    return () => setIsFabHidden(false);
  }, [selectedItem, outOfStockItem, quantityItem, setIsFabHidden]);

  // Watch for order status changes → fire notification toast
  const prevOrderStatuses = useRef<Map<string, string>>(new Map());
  useEffect(() => {
    if (!orders?.length) return;
    orders.forEach(order => {
      const prev = prevOrderStatuses.current.get(order.orderId);
      if (prev !== undefined && prev !== order.orderStatus) {
        const shortId = order.orderId.slice(-4).toUpperCase();
        if (order.orderStatus === 'Đã thanh toán') {
          showToast(`✅ Ord #${shortId} đã được thanh toán thành công`);
        } else if (order.orderStatus === 'Hoàn thành') {
          showToast(`☕ Ord #${shortId} đã pha chế xong`);
        }
      }
      prevOrderStatuses.current.set(order.orderId, order.orderStatus);
    });
  }, [orders]);
  ...
  // Auto-hide timeAgo badge 5s after refresh
  useEffect(() => {
    if (!lastUpdated) return;
    setShowTimeAgoBadge(true);
    const timer = setTimeout(() => setShowTimeAgoBadge(false), 5000);
  ...
  // Card click → instant add qty 1 (no popup)
  const handleOpenModal = useCallback((item: MenuItem) => {
    if (item.isOutOfStock) {
      setOutOfStockItem(item as GroupedMenuItem);
      return;
    }
  ...
  }, [addToCart, getCartQuantity, showToast]);

  // + button click → CustomizationModal trực tiếp
  const handleAddQuick = useCallback((e: React.MouseEvent, item: MenuItem) => {
    ...
    setSelectedItem(item as GroupedMenuItem);
  }, []);
      ...
      <div className="flex flex-col min-h-full bg-stone-50 dark:bg-black">
        ...
        <div className="pt-[132px] lg:pt-4 px-4 pb-24 lg:pb-8 space-y-3 animate-pulse">
          {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
            <div
              key={i}
              className="bg-white dark:bg-stone-900 rounded-2xl p-4 flex items-center justify-between gap-4 border border-stone-100 dark:border-stone-800 shadow-sm"
              style={{ opacity: 1 - i * 0.08 }}
            >
              <div className="flex-1 min-w-0 space-y-2">
                <div className="h-4 bg-stone-100 dark:bg-stone-800 rounded-lg w-2/3" />
                <div className="h-4 bg-stone-100 dark:bg-stone-800 rounded-lg w-1/3" />
              </div>
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 bg-stone-100 dark:bg-stone-800 rounded-full" />
                <div className="w-11 h-11 bg-stone-100 dark:bg-stone-800 rounded-2xl" />
              </div>
            </div>
          ))}
        </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[70vh] text-center px-6">
        <motion.div 
          ...
          className="w-24 h-24 bg-red-50 dark:bg-red-900/20 text-red-500 rounded-[40px] flex items-center justify-center mb-8 shadow-xl shadow-red-500/10"
        >
          <AlertCircle className="w-12 h-12" />
        </motion.div>
        <h2 className="text-2xl font-black text-stone-800 dark:text-white mb-3 tracking-tight">Không thể tải thực đơn</h2>
        <p className="text-stone-500 dark:text-stone-400 mb-10 max-w-xs text-sm leading-relaxed font-medium">
          {error.includes('Rate Limit') 
            ? 'Hệ thống đang bận do có quá nhiều người truy cập cùng lúc. Vui lòng đợi 1-2 phút rồi nhấn nút Thử lại.'
            : 'Có lỗi xảy ra khi kết nối với hệ thống. Vui lòng kiểm tra lại đường dẫn Apps Script trong phần Cài đặt.'}
        </p>
        <div className="w-full max-w-xs space-y-4">
          <button
            onClick={() => fetchAllData(true)}
            className="w-full py-4.5 bg-[#C9252C] text-white font-black text-xs uppercase tracking-widest rounded-2xl tap-active shadow-lg shadow-red-900/20 flex items-center justify-center gap-2 active:scale-95 transition-all"
          >
          ...
          <button
            onClick={onNavigateSettings}
            className="w-full py-4.5 bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-400 font-black text-xs uppercase tracking-widest rounded-2xl tap-active hover:bg-stone-200 dark:hover:bg-stone-700 transition-colors"
          >
            Cấu hình lại Apps Script
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="flex flex-col min-h-full lg:h-full lg:w-full lg:min-h-0 min-w-0">
      ...
      {error && menuItems.length > 0 && (
        <div
          key="error-toast"
          className="fixed bottom-[160px] left-4 right-4 z-[60]"
        >
...
const MenuItemCard = memo(({ 
  ...
  const hasOptions = item.hasCustomizations !== false;

  const handleItemClick = (e: React.MouseEvent) => {
    if (item.isOutOfStock) return;
    onOpenModal(); // always instant-add
  };
      ...
      <div className="mb-2">
        <div className="flex justify-between items-start gap-2 mb-1">
           <div className="flex-1 min-w-0">
            ...
            {/* Inventory qty pill - always shown when available */}
            {!item.isOutOfStock && item.inventoryQty !== undefined && (
              <span
...
const CustomizationModal: React.FC<{ item: GroupedMenuItem; currentQty: number; initialQuantity?: number; onClose: () => void; onAdd: (item: CartItem, e: React.MouseEvent) => void; showToast: (msg: string, type?: 'success' | 'warning') => void }> = ({ item, currentQty, initialQuantity = 1, onClose, onAdd, showToast }) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="px-6 pt-2 pb-3 border-b border-stone-50 dark:border-stone-800/50">
          <p className="text-[10px] font-black text-stone-400 dark:text-stone-500 uppercase tracking-widest mb-0.5">Số lượng</p>
          <h2 className="text-xl font-black text-[#C9252C] uppercase tracking-tight">{item.name}</h2>
          <p className="text-sm font-bold text-stone-500 dark:text-stone-400 mt-0.5">{item.price.toLocaleString()}đ / ly</p>
        </div>
        <div className="px-6 py-6 flex items-center justify-between">
          <div className="flex items-center bg-stone-50 dark:bg-stone-950 rounded-xl p-0.5 border border-stone-100 dark:border-stone-800">
            <button
              ...
              className="w-11 h-11 bg-white dark:bg-stone-800 rounded-lg flex items-center justify-center text-stone-500 tap-active shadow-sm border border-stone-100 dark:border-stone-700"
            >
              <Minus className="w-5 h-5" />
            </button>
            <span className="w-14 text-center font-black text-2xl text-stone-800 dark:text-white">{quantity}</span>
            <button
          ...
          <div className="text-right">
            <p className="text-stone-400 text-[9px] font-black uppercase tracking-widest mb-0.5">Tổng</p>
            <p className="text-2xl font-black text-[#C9252C] tracking-tight">{(item.price * quantity).toLocaleString()}đ</p>
          </div>
        </div>
        <div className="px-6 pb-6">
          <button
            onClick={() => onConfirm(quantity)}
            className="w-full py-4 bg-[#C9252C] text-white font-black text-[15px] uppercase tracking-wider rounded-2xl tap-active shadow-lg shadow-red-200 dark:shadow-none flex items-center justify-center gap-2"
          >
            Xác nhận
          </button>
        </div>

 ...
 *   A: ORDER_ID       B: TIMESTAMP      C: TABLE_NO
 *   D: ITEMS          E: SUBTOTAL       F: DISCOUNT
 *   G: VAT_AMOUNT     H: THANH_TIEN     I: TRANG_THAI
...
function getOrdersData() {
  ...
  rows.forEach(function (row) {
    ...
    orderList.push({
      ...
      THANH_TIEN: Number(row[7]) || 0, // Tổng tiền thực thu
...
function createNewOrder(payload) {
  ...
  sheet.appendRow([
    ...
    thanhTien,       // Cột H: THANH_TIEN ← FIX: không còn = 0 nữa
    trangThai,       // Cột I: TRANG_THAI
...
function updateOrderStatus(payload) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0].toString().trim() === orderIdCanTim.toString().trim()) {
      ...
      // Cập nhật TRANG_THAI → Cột I = cột số 9
      ...
      if (trangThaiMoi === "Đã hủy") {
        try {
          const thanhTienHuy = Number(data[i][7]) || 0; // cột H = THANH_TIEN
 ...
 *   thanhTien {number}  Tổng tiền thực thu (THANH_TIEN)
...
function addItemsToOrder(payload) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    const thanhTienCu = Number(data[i][7]) || 0; // Cột H: THANH_TIEN
    ...
    sheet.getRange(hang, 8).setValue(thanhTienMoi);                  // Cột H: THANH_TIEN
...
function editOrder(payload) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    // Cập nhật tổng tiền (cột E = SUBTOTAL, cột H = THANH_TIEN)

...
function getStaffData() {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0]) { // ma_nv
      staffList.push({
        ma_nv: data[i][0].toString(),
        ten_nv: data[i][1].toString(),
        sdt: data[i][2].toString(),
        vi_tri: data[i][3],
        loai_hinh: data[i][4],
        muc_luong: Number(data[i][5]) || 0,
        ngay_vao_lam: data[i][6],
        trang_thai: data[i][7] === true // Checkbox đang làm việc
      });
    }
  }
  return staffList;
 ...
 * @param {Object} payload { action: "attendance", ma_nv: "NV01", type: "in"/"out" }
 */
function handleAttendance(payload) {
  ...
  if (payload.type === "in") {
    ...
    for (let i = data.length - 1; i >= 1; i--) {
      if (data[i][2] === payload.ma_nv && data[i][1] === dateStr && data[i][4] === "") {
        const row = i + 1;
...
function updateStaffStatus(payload) {
  ...
  for (let i = 1; i < data.length; i++) {
    if (data[i][0].toString() === payload.ma_nv) {
      const row = i + 1;
      if (payload.trang_thai !== undefined) {
        sheet.getRange(row, 8).setValue(payload.trang_thai); // Cột H
      }
      return { message: "Đã cập nhật nhân sự " + payload.ma_nv };
    }
  }
